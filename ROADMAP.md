...
- **Authoring agents/workflows in Python or TypeScript as canonical
  ...
  **[2026-09-01 — argued narrowing, ADR-0071.]** The kill is *narrowed,
  not lifted*: it stands for the **vocabulary** (agents, models, tools,
  `[allow]` — TOML/dirs and Rust attributes only, forever), and is
  **upheld** there. It no longer covers **choreography**: `pipelines/*.ts`
  is now a canonical authoring surface for flow, synthesized to
  ProjectConfig JSON through the same single validator (the compiler
  thesis is preserved — TS runs only at synth time, never at runtime, and
  the constitution is never generated by the thing it governs). See the
  three-surface split in
  [2026-09-01-tau-authoring-ops-and-primitives-design.md](docs/superpowers/specs/2026-09-01-tau-authoring-ops-and-primitives-design.md) §1.
  Killed items change by argued amendment like this one — never silently.
...
- **Adds:** `@tau/sdk` package shape — `agent({...})`, `tool({...})`,
  ...
  choice), one IR underneath. *[2026-09-01: the "TOML or TS, your
  choice" framing is retired by the three-surface split (ADR-0071) —
  surfaces are complementary by category (TOML = vocabulary, TS =
  choreography, Rust = muscle), not interchangeable front-ends. "One IR
  underneath" stands.]*
- **Supersedes:** nothing.
...
- Inline TS tool bodies (`run: async () => ...`) — *[2026-09-01: killed,
  never lands. JS at runtime / QuickJS-embedded tool bodies are rejected
  by the three-surface split (ADR-0071; design §8 rejections ledger) —
  the engine stays 100% Rust, TS runs only at synth time. Tool bodies
  are Rust (`#[tau::tool]`).]*
...
*[2026-09-01: re-pointed by the three-surface split (ADR-0071). "TS is
sugar" is dropped — TS is the canonical choreography surface
(`pipelines/*.ts`, synth-time only), TOML the vocabulary, Rust the
muscle. The δ.2 polish items fold into the redesign epics E-1/E-2 (see
`docs/superpowers/plans/vision-roadmap.md`); the QuickJS/runtime-JS item
is killed outright (design §8).]*
