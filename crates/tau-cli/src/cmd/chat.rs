...
pub async fn run(
    ...
    // Host-shell contract (`stream.rs::clock_ref`): the chat shell drives the
    // core streaming path directly, so it must inject the production clock +
    // randomness or `run_streaming_inner` panics on the first port use. The
    // IR/bundle paths inject these via the dispatcher instead.
