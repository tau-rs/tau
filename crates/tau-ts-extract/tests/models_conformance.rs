//! TOML↔TS conformance for the `[models]` table (per-agent model resolution).
//! A multi-alias `[models]` table with two agents on different models must
//! produce byte-equal canonical IR regardless of authoring surface — proving
//! the TS `models({...})` factory + per-agent `model` alias lowers identically
//! to the TOML `[models]` table + `agent.model`.

use std::path::Path;

#[test]
fn toml_and_ts_produce_byte_equal_canonical_ir_with_models_table() {
    let fixture_dir =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/models_conformance");
    ...
    let deep = ts_ir
        .workflow
        .agents
        .get(&tau_ir::ids::AgentId("deep".into()))
        .expect("deep agent");
    let fast = ts_ir
        .workflow
        .agents
        .get(&tau_ir::ids::AgentId("fast".into()))
        .expect("fast agent");
    assert_eq!(deep.model_ref.model_id, "claude-opus-4-8");
    assert_eq!(fast.model_ref.model_id, "claude-haiku-4-5");
