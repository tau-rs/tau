...
fn schema_path() -> PathBuf {
    ...
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../schemas/ir/tau-ir.v2.7.0.schema.json")
}
...
fn schema_matches_checked_in_file() {
    ...
    let on_disk = std::fs::read_to_string(schema_path())
        .expect("schemas/ir/tau-ir.v2.7.0.schema.json missing — run with UPDATE_SCHEMA=1");
    assert_eq!(
        generated, on_disk,
