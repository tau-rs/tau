...
//! - `MessagePayload` has bidirectional `From` adapters for `tau_domain::MessagePayload`
//!   (alloc-only; no std required).
//! - The `Message` envelope adapters (which require `std::time::SystemTime`) live
//!   in `tau-ir-lower` where std is available.
