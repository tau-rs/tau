    ...
    for step in &pipeline.steps {
        ...
        if !exists {
            ...
            let target = match &step.run {
                StepRun::Agent(a) => format!("agent:{}", a.0),
                StepRun::Tool(t) => format!("tool:{}", t.0),
                StepRun::Deterministic(s) => format!("deterministic:{}", s.0),
                StepRun::Check(c) => format!("check:{}", c.0),
            };
...
mod tests {
    ...
    fn agent_with_context(steps: Vec<&str>) -> tau_ir::module::Workflow {
        let pipeline = steps
            .into_iter()
            .map(|t| ContextStep {
                transformer: t.into(),
    ...
    fn context_ok_when_fit_budget_last() {
        let wf = agent_with_context(vec!["trim_old", "fit_budget"]);
        assert!(check_context(&wf).is_ok());
    }

    #[test]
    fn context_rejects_unknown_transformer() {
        let wf = agent_with_context(vec!["bogus", "fit_budget"]);
        assert!(matches!(
            check_context(&wf),
    ...
    fn context_rejects_fit_budget_not_last() {
        let wf = agent_with_context(vec!["fit_budget", "trim_old"]);
        assert!(matches!(
            check_context(&wf),
    ...
    fn context_rejects_duplicate() {
        let wf = agent_with_context(vec!["trim_old", "trim_old", "fit_budget"]);
        assert!(matches!(
            check_context(&wf),
