...
pub(super) struct Parsed {
    ...
    pub(super) triggers: Vec<TriggerBinding>,
}
...
pub(super) fn parse(config: &ProjectConfig) -> Result<Parsed, LowerError> {
    ...
    let edges: Vec<SubflowEdge> = Vec::new();
    ...
    // --- Triggers (slice 1) --------------------------------------------
    let mut triggers: Vec<TriggerBinding> = Vec::new();
...
fn lower_checks(
    ...
    let mut appended: Vec<PipelineStep> = Vec::new();
