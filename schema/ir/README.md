...
agent + MCP tool, a deterministic step, a subflow, and a tool whose impl is
a deterministic `Step` (`tool_impl_step.json`). They are regenerated by
`cargo xtask gen-ir-schema` and are validated against the schema in CI.
