...
## What you author: a manifest, not a language

tau is a manifest plus a small amount of native tool code. A typical agent in
TOML:
...
The eventual TS sugar layer (post-IR) emits the same IR, type-checked:

```ts
import { agent, tool, mcp, contextManager } from "tau";

const readTemp = tool({
  description: "Read the temperature sensor",
  capabilities: { hardware: ["i2c:0x48"] },
  run: async () => readSensor(),
});
...
export const fanMonitor = agent({
  ...
  context: contextManager({
    ...
    pipeline: ["trimOld", "compactToolOutputs", "summarizeOldest", "fitBudget"],
  }),
});
```

**The TS surface is sugar over the IR, not a parallel runtime.** The IR is
canonical; both surfaces lower to it.
