...
- **2026-09-26, later** — [#223](https://github.com/tau-rs/tau/issues/223)
  ...
  - **unknown `session`** — `exec resume` of an id with no rollout prints
    ...
    ceiling — a run that never started, billed as one that vanished. The
    adapter now refuses before `settle` a run that ended by itself having
    printed nothing, with a non-zero exit: `error.provider`, the exit code
