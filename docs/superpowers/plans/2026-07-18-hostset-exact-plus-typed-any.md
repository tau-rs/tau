...
- **ADR number: 0064.** Conventional commits, imperative, scoped.
...
- **Create** `docs/decisions/0064-one-host-semantic-exact-typed-any.md`; **Modify** `docs/SUMMARY.md`, `capability.rs:137` doc comment.
...
**Files:** Create `docs/decisions/0064-one-host-semantic-exact-typed-any.md`; Modify `docs/SUMMARY.md`, `crates/tau-domain/src/package/capability.rs` (doc comment), migration targets.
...
- [ ] **Step 2: Write ADR 0064**

Create `docs/decisions/0064-one-host-semantic-exact-typed-any.md` following the repo's ADR template (status Accepted; context = the three-way divergence; decision = `HostSet` exact + typed `Any`, no derived hosts, typed `HttpMethod`, absent-vs-empty; consequences = build-accepts ⟺ run-enforces; deferred = suffix wildcards, IPv6 literals, runtime method enforcement). Reference the spec.
...
- `docs/decisions/0019-per-host-network-filter.md:105` — add a one-line "Superseded in part by ADR-0064 (hosts are now `HostSet`; `\"*\"` is a parse error, `\"any\"` is the sentinel)."
...
git commit -m "docs(adr-0064): one host semantic; migrate methods=[] fixtures + mcp examples"
```
...
gh pr create --base main --title "feat: one host semantic — HostSet exact + typed any (ADR-0064)" --body "Implements docs/superpowers/specs/2026-07-18-hostset-exact-plus-typed-any-design.md. Closes the hosts=['*'] build-accepts-but-run-rejects divergence."
```
