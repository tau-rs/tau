...
**ADR:** ADR-0061 (this design) lands in PR 1; credential-flip ADR at PR 4 (next-free then)
...
|---|---|---|
| **1** | `link()` + `LinkRecord` + ADR-0061 | New `tau-pkg/src/link.rs`; `LinkRecord`/`LinkError`; truth-table + multi-error tests. ADR + SUMMARY.md. Nothing calls it yet (unit-tested dead code). |
...
- ADR-0061 "tau build links; bundles carry a verified LinkRecord; run trusts
